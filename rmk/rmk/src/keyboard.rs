    ...
    Keyboard<'a, ROW, COL, NUM_LAYER, NUM_ENCODER>
{
    ...
    async fn fire_held_keys(
        ...
        for (pos, decision) in decisions {
            ...
            // debug!("✁EDecision for held key: {:?}: {:?}", pos, decision)
            match decision {
    ...
    pub fn print_buffer(&self) {
        self.held_buffer
            ...
            .iter()
            .enumerate()
            .for_each(|(i, k)| info!("\n✁Eeld buffer {}: {:?}, state: {:?}", i, k.event, k.state));
    }
    ...
    async fn process_user(&mut self, key: KeyCode, event: KeyboardEvent) {
        ...
        if id >= 10 {
            let note = 60 + (id - 10);
            if event.pressed {
                let _ = crate::channel::MIDI_CHANNEL.try_send([0x09, 0x90, note, 0x7F]);
            } else {
                let _ = crate::channel::MIDI_CHANNEL.try_send([0x08, 0x80, note, 0x00]);
            }
            return;
        }
        #[cfg(feature = "_ble")]
