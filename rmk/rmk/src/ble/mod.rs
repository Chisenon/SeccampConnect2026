...
>(
    ...
    #[cfg(not(feature = "_no_usb"))]
    let (mut _usb_builder, mut keyboard_reader, mut keyboard_writer, mut other_writer, midi_class) = {
        ...
        let midi_class = embassy_usb::class::midi::MidiClass::new(&mut usb_builder, 1, 1, 64);
        let (keyboard_reader, keyboard_writer) = keyboard_reader_writer.split();
        (usb_builder, keyboard_reader, keyboard_writer, other_writer, midi_class)
    };
    ...
    #[cfg(not(feature = "_no_usb"))]
    let usb_midi_task = async {
        let (mut midi_tx, _) = midi_class.split();
        loop {
            midi_tx.wait_connection().await;
            loop {
                let packet = crate::channel::MIDI_CHANNEL.receive().await;
                if midi_tx.write_packet(&packet).await.is_err() {
                    break;
                }
            }
        }
    ...
    let background_task = embassy_futures::join::join3(ble_task(runner), usb_task, usb_midi_task);
    #[cfg(all(feature = "usb_log", not(feature = "_no_usb")))]
    let background_task = embassy_futures::join::join3(
        ble_task(runner),
        select(
            usb_task,
